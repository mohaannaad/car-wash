...
const weekDays = [
  { index: 0, name: "أحد" }, { index: 1, name: "اثنين" }, { index: 2, name: "ثلاثاء" },
  { index: 3, name: "أربعاء" }, { index: 4, name: "خميس" }, { index: 5, name: "جمعة" }, { index: 6, name: "سبت" },
];
...
function generateScheduleDates(dayOfWeek: number, count: number) {
  ...
  while (dates.length < count) {
    if (cursor.getDay() === dayOfWeek) {
      dates.push({ key: cursor.toISOString().split("T")[0], label: `${cursor.getDate()} ${monthNames[cursor.getMonth()]}` });
...
export default function SubscriptionSchedulePage() {
  ...
  const washCount = subscription.package?.washCount ?? 4;
      ...
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-6 px-5">
            ...
            {weekDays.map((day) => (
              <button
                ...
                className={`rounded-xl py-3 text-sm font-bold transition-all ${
                  selectedDay === day.index ? "bg-primary text-white shadow-[0_6px_16px_rgba(25,185,198,0.3)]" : "bg-white text-text-main shadow-[0_2px_10px_rgba(16,24,40,0.05)]"
                }`}
              >
                {day.name}
              </button>
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {timeSlots.map((time) => (
              <button
                ...
                className={`rounded-xl py-3 text-sm font-bold transition-all ${
                  selectedTime === time ? "bg-primary text-white shadow-[0_6px_16px_rgba(25,185,198,0.3)]" : "bg-white text-text-main shadow-[0_2px_10px_rgba(16,24,40,0.05)]"
                }`}
              >
                {time}
              </button>
        ...
        {selectedDay !== null && (
          <div className="bg-white rounded-2xl p-4 flex flex-col gap-3 shadow-[0_2px_10px_rgba(16,24,40,0.05)]">
            <span className="text-text-main text-sm font-bold">مواعيد الغسيل هذا الشهر ({washCount})</span>
            <div className="flex flex-wrap gap-2">
              {previewDates.map((d) => (
                <span key={d.key} className="bg-primary-light text-primary text-xs font-bold px-3 py-1.5 rounded-full">{d.label}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="px-5 pt-4 pb-8 shrink-0">
        <button
          ...
          className={`w-full py-4 rounded-2xl font-bold text-white transition-colors ${
            selectedDay !== null && selectedTime ? "bg-primary shadow-[0_8px_20px_rgba(25,185,198,0.35)]" : "bg-disabled cursor-not-allowed"
          }`}
        >
          تأكيد الاشتراك
        </button>
